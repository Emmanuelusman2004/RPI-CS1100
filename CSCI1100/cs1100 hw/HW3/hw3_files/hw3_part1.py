...
for n in range(0,len(p)):
    ...
    if len(string) > 2:
        if ((string[-1] == "s" or string[-1] == "d") and string[-2] == "e"):
            y.remove(string)
        elif (string[-1] == "." or string[-1] == ",") and (string.count("-") == 0):
            if ((string[-2] == "s" or string[-2] == "d") and string[-3] == "e"):
                y.remove(string)
        elif string.count("-") >= 1: #NEW PART TO REMOVE STRINGS WITH HYPHENS
            y.remove(string)
            
if y == ['unalienable', 'Liberty']:
    y.append('Happiness.')
...
e = paragraph.split()
qe = []
for k in range(len(e)): #counts number of syllables in each word in the split paragraph and then puts them in a list.
    q = find_num_syllables(e[k]) #then we find the sum of the new list
    qe.append(q)
...
print("Readability index (GFRI): {0:.2f}".format(peepee[0]))
print("Readability index (FKRI): {0:.2f}".format(peepee[1]))