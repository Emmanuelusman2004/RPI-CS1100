...
Keyboardfile = input("Keyboardfile => ")
...
#swap
            i = 0
            count = 0
            for char in lineword:
                indexofchar = lineword.index(char)
                indexofnextchar = indexofchar+1
                splitword = [x for x in lineword]
                splitword.pop(indexofchar)
                splitword.insert(indexofnextchar+1,char)
                splitwordcombined = ''.join(splitword)
                if splitwordcombined in d:
                    count+=1
                    wordsfound.add(splitwordcombined)
...
#not found
            if wordsfound == set():
                ...
                wordsfound = Sorttuples(wordsfoundnumbers)
                stringwordsfound = ''
                for tup in reversed(wordsfound[-3:]):
                    stringwordsfound+= " " + tup[0]
                print('{0:>15} -> FOUND {1:>3}: {2}'.format(line,len(wordsfound),stringwordsfound))
